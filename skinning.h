...
public:
  ...
  void setSkinningMode(SkinningMode mode) { skinningMode = mode; }
  SkinningMode getSkinningMode() const { return skinningMode; }

  // Accessors used by vertex-handle IK.
  int getNumMeshVertices() const { return numMeshVertices; }
  int getNumJointsInfluencingEachVertex() const { return numJointsInfluencingEachVertex; }
  const int * getMeshSkinningJoints() const { return meshSkinningJoints.data(); }
  const double * getMeshSkinningWeights() const { return meshSkinningWeights.data(); }
  const double * getRestMeshVertexPositions() const { return restMeshVertexPositions; }
...
protected:
  ...
  SkinningMode skinningMode = LINEAR_BLEND;
