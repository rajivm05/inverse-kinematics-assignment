...
{
  int numJoints = (int)translations.size();

  // First, compute each joint's local transform.
  // Maya convention: localRotation = R_jointOrient * R_eulerAngles.
  // jointOrientation is always in XYZ order.
  for (int i = 0; i < numJoints; i++)
  {
    double eulerR[9];
    euler2Rotation(eulerAngles[i].data(), eulerR, rotateOrders[i]);

    double orientR[9];
    euler2Rotation(jointOrientationEulerAngles[i].data(), orientR, XYZ);

    Mat3d localRotation = asMat3d(orientR) * asMat3d(eulerR);
    localTransforms[i] = RigidTransform4d(localRotation, translations[i]);
  }

  // Now traverse joints in update order (parents before children) to build globals.
  for (int k = 0; k < numJoints; k++)
  {
    int jointID = jointUpdateOrder[k];
    int parentID = jointParents[jointID];
    if (parentID < 0)
      globalTransforms[jointID] = localTransforms[jointID];
    else
      globalTransforms[jointID] = RigidTransform4d(globalTransforms[parentID] * localTransforms[jointID]);
  }
}
...
{
  for (size_t i = 0; i < skinTransforms.size(); i++)
  {
